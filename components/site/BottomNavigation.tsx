...
function Internal({ href, label, Icon, active }: { href: string; label: string; Icon: typeof Home; active: boolean }) {
  return (
    <Link href={href} data-active={active} className="relative flex min-w-0 flex-1 flex-col items-center justify-center gap-0.5 rounded-2xl px-1 py-2 text-[9px] font-semibold transition">
      {active && <span className="absolute inset-1 -z-10 rounded-[45%_55%_50%_40%] bg-gold-gradient shadow-gold animate-[liquidPop_.4s_ease]" />}
