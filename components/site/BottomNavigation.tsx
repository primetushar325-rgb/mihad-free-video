...
export default function BottomNavigation() {
  ...
  useEffect(() => {
    ...
    indicator.style.width = itemRect.width * 0.92 + "px";
    indicator.style.height = itemRect.height * 0.92 + "px";
    indicator.style.opacity = "1";
    indicator.style.transform = `translate(${x}px, ${y}px)`;
    indicator.style.borderRadius = "50% 40% 55% 45%";
    const t = setTimeout(() => { indicator.style.borderRadius = "45% 55% 50% 40%"; }, 350);
  ...
  return (
    <>
      ...
      <nav ref={navRef} aria-label="Primary mobile navigation" className="glass-strong pointer-events-auto fixed inset-x-2 bottom-[calc(.5rem+env(safe-area-inset-bottom))] z-50 mx-auto flex touch-manipulation max-w-md items-center justify-around rounded-[24px] border border-gold-500/25 px-1 py-1.5 shadow-2xl md:hidden">
        <div ref={indicatorRef} className="liquid-indicator pointer-events-none absolute z-0" style={{ opacity: 0, willChange: "transform,border-radius" }} />
